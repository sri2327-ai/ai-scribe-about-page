            ...
            <Card className={`bg-white border-0 border-gray-100 transition-all duration-300 overflow-hidden ${shadowStyles.brandGlow} ring-1 ring-gray-100/70 backdrop-blur-sm hover:-translate-y-1 dark:bg-gray-900/95 dark:border-gray-800`}>
              ...
              <Tabs defaultValue="ai-scribe" className="w-full">
                <div className="px-3 sm:px-4 pt-3 sm:pt-4">
                  {/* Enhanced tab navigation with better font size for desktop views */}
                  <TabsList className="w-full grid grid-cols-2 md:grid-cols-4 bg-gray-50/70 p-1 rounded-lg dark:bg-gray-800/30">
                    {featureTabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="flex items-center justify-center px-2 py-2 md:py-1.5 gap-1 sm:gap-1.5 my-1 text-center rounded-lg data-[state=active]:shadow-none text-xs md:text-[0.7rem] font-medium md:whitespace-normal dark:text-gray-300 dark:data-[state=active]:text-white">
                        <span className="flex items-center justify-center shrink-0">{tab.icon}</span>
                        <span className="md:text-[0.7rem] md:leading-tight">{tab.title}</span>
                      </TabsTrigger>)}
                  </TabsList>
                </div>
