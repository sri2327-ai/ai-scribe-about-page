...
const LanguageGrid = React.memo(() => (
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 text-xs sm:text-sm">
    {supportedLanguages.map((lang, index) => (
      <motion.div
        ...
        className="flex items-center gap-1.5 bg-white p-2 rounded-lg shadow-sm"
      >
        <CheckCircle className="w-3 h-3 text-green-500 flex-shrink-0" />
        <span className="text-gray-700 font-medium truncate">{lang}</span>
      </motion.div>
    ))}
  </div>
));
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden hidden sm:block">
            <div className="absolute -top-10 -right-10 w-20 h-20 sm:w-32 sm:h-32 bg-gradient-to-br from-blue-200 to-teal-200 rounded-full opacity-30 animate-pulse"></div>
            <div className="absolute -bottom-10 -left-10 w-24 h-24 sm:w-40 sm:h-40 bg-gradient-to-br from-purple-200 to-pink-200 rounded-full opacity-20 animate-pulse delay-1000"></div>
          </div>

          {/* Header */}
          <div className="bg-gradient-to-r from-[#143151] via-[#387E89] to-[#143151] text-white p-4 sm:p-6 lg:p-8 text-center relative overflow-hidden">
            ...
            >
              <h2 className="text-lg sm:text-2xl lg:text-3xl xl:text-4xl font-bold mb-2 sm:mb-3 leading-tight">{content.title}</h2>
              <p className="text-blue-100 text-sm sm:text-base lg:text-lg xl:text-xl font-medium">{content.subtitle}</p>
            </motion.div>
          </div>

          {/* Main content */}
          <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8 relative z-10">
            {/* Key benefits */}
            <motion.div
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              ...
              {content.features.map((feature, index) => (
                ...
                >
                  <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                    <div className={`p-2 sm:p-3 bg-${feature.color}-100 rounded-xl shadow-sm`}>
                      <feature.icon className={`w-4 h-4 sm:w-6 sm:h-6 text-${feature.color}-600`} />
                    </div>
                    <h3 className="font-bold text-sm sm:text-lg lg:text-xl text-gray-800">{feature.title}</h3>
                  </div>
                  <p className="text-xs sm:text-sm lg:text-base text-gray-600 leading-relaxed mb-3 sm:mb-4">{feature.description}</p>
            ...
            {showLanguages && (
              <motion.div
                ...
                className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-4 sm:p-6 max-h-48 sm:max-h-64 overflow-y-auto border border-gray-200"
              >
            ...
            <motion.div
              ...
              className="text-center bg-gradient-to-r from-blue-50 via-white to-teal-50 rounded-xl p-4 sm:p-6 lg:p-8 border border-blue-200"
            >
              <h3 className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-bold text-gray-800 mb-2 sm:mb-3">
                {content.cta}
              </h3>
              <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base lg:text-lg leading-relaxed max-w-2xl mx-auto">
            ...
            >
              <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 text-xs sm:text-sm text-gray-600 bg-white/80 rounded-full px-4 sm:px-6 py-2 sm:py-3 border border-gray-200 shadow-sm">
